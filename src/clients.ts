...
export async function makeClobClient() {
  const provider = new ethers.providers.JsonRpcProvider('https://polygon-rpc.com', CFG.chainId);
  ...
  try {
    ...
    console.warn('[CLOB] deriveApiKey failed; will try create', e?.response?.data || e);
  }
  if (!creds) {
    try {
      ...
      console.warn('[CLOB] createApiKey failed; continuing without cached creds', e?.response?.data || e);
    }
  }
